## Пофиксить баги
Для начала попросить оценить данный код, почекать потенциальные ошибки, пофиксить баги.

(Второе задание по желанию) конвертировать из closure’оф в combine. Дана схема на UIKit + Delegate где вся логика во вью контроллере. Задача - перенести логику из контроллера в ViewModel (MVVM) + Сделать общение через Combine.
